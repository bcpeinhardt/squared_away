...
-spec one_of(list(MKM)) -> fun((MKM) -> {ok, MKM} | {error, snag:snag()}).
one_of(Allowed) ->
...
-spec none_of(list(MKP)) -> fun((MKP) -> {ok, MKP} | {error, snag:snag()}).
none_of(Disallowed) ->
...
-spec each(fun((MKS) -> {ok, MKS} | {error, snag:snag()})) -> fun((list(MKS)) -> {ok,
        list(MKS)} |
    {error, snag:snag()}).
each(Constraint) ->
