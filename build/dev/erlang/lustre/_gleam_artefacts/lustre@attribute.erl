...
-spec on(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, QSO} |
        {error, list(gleam@dynamic:decode_error())})
) -> lustre@internals@vdom:attribute(QSO).
on(Name, Handler) ->
    {event, <<"on"/utf8, Name/binary>>, Handler}.

-spec map(lustre@internals@vdom:attribute(QST), fun((QST) -> QSV)) -> lustre@internals@vdom:attribute(QSV).
map(Attr, F) ->
    case Attr of
