...
-opaque app(SQV, SQW, SQX) :: {app,
        fun((SQV) -> {SQW, lustre@effect:effect(SQX)}),
        fun((SQW, SQX) -> {SQW, lustre@effect:effect(SQX)}),
        fun((SQW) -> lustre@internals@vdom:element(SQX)),
...
-spec application(
    fun((SRQ) -> {SRR, lustre@effect:effect(SRS)}),
    fun((SRR, SRS) -> {SRR, lustre@effect:effect(SRS)}),
    fun((SRR) -> lustre@internals@vdom:element(SRS))
) -> app(SRQ, SRR, SRS).
application(Init, Update, View) ->
    {app, Init, Update, View, none}.

-spec element(lustre@internals@vdom:element(SRE)) -> app(nil, nil, SRE).
element(Html) ->
...
-spec simple(
    fun((SRJ) -> SRK),
    fun((SRK, SRL) -> SRK),
    fun((SRK) -> lustre@internals@vdom:element(SRL))
) -> app(SRJ, SRK, SRL).
...
-spec component(
    fun((SRZ) -> {SSA, lustre@effect:effect(SSB)}),
    fun((SSA, SSB) -> {SSA, lustre@effect:effect(SSB)}),
    fun((SSA) -> lustre@internals@vdom:element(SSB)),
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, SSB} |
        {error, list(gleam@dynamic:decode_error())}))
) -> app(SRZ, SSA, SSB).
component(Init, Update, View, On_attribute_change) ->
    {app, Init, Update, View, {some, On_attribute_change}}.

-spec do_start(app(SSV, any(), SSX), binary(), SSV) -> {ok,
        fun((lustre@internals@runtime:action(SSX, client_spa())) -> nil)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.

-spec do_start_actor(app(SUA, any(), SUC), SUA) -> {ok,
        gleam@erlang@process:subject(lustre@internals@runtime:action(SUC, server_component()))} |
    {error, error()}.
...
-spec start_actor(app(STP, any(), STR), STP) -> {ok,
        gleam@erlang@process:subject(lustre@internals@runtime:action(STR, server_component()))} |
    {error, error()}.
start_actor(App, Flags) ->
    do_start_actor(App, Flags).

-spec start_server_component(app(STF, any(), STH), STF) -> {ok,
        fun((lustre@internals@runtime:action(STH, server_component())) -> nil)} |
    {error, error()}.
...
-spec dispatch(SUS) -> lustre@internals@runtime:action(SUS, any()).
dispatch(Msg) ->
    {dispatch, Msg}.
...
-spec start(app(SSL, any(), SSN), binary(), SSL) -> {ok,
        fun((lustre@internals@runtime:action(SSN, client_spa())) -> nil)} |
    {error, error()}.
