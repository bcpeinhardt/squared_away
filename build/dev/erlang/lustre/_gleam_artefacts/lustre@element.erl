...
-spec element(
    ...
    list(lustre@internals@vdom:element(RAL))
) -> lustre@internals@vdom:element(RAL).
...
-spec do_keyed(lustre@internals@vdom:element(RAY), binary()) -> lustre@internals@vdom:element(RAY).
do_keyed(El, Key) ->
    case El of
...
-spec keyed(
    fun((list(lustre@internals@vdom:element(RAR))) -> lustre@internals@vdom:element(RAR)),
    list({binary(), lustre@internals@vdom:element(RAR)})
) -> lustre@internals@vdom:element(RAR).
keyed(El, Children) ->
    El(
        (gleam@list:map(
            Children,
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@internals@vdom:attribute(RBB)),
    list(lustre@internals@vdom:element(RBB))
) -> lustre@internals@vdom:element(RBB).
...
-spec advanced(
    binary(),
    binary(),
    list(lustre@internals@vdom:attribute(RBH)),
    list(lustre@internals@vdom:element(RBH)),
    boolean(),
    boolean()
) -> lustre@internals@vdom:element(RBH).
...
-spec flatten_fragment_elements(list(lustre@internals@vdom:element(RBV))) -> list(lustre@internals@vdom:element(RBV)).
...
-spec fragment(list(lustre@internals@vdom:element(RBR))) -> lustre@internals@vdom:element(RBR).
fragment(Elements) ->
    _pipe = flatten_fragment_elements(Elements),
    {fragment, _pipe, <<""/utf8>>}.

-spec map(lustre@internals@vdom:element(RBZ), fun((RBZ) -> RCB)) -> lustre@internals@vdom:element(RCB).
map(Element, F) ->
    case Element of
        {text, Content} ->
            {text, Content};
