...
-type patch(RGD) :: {diff, element_diff(RGD)} |
    {emit, binary(), gleam@json:json()} |
    {init, list(binary()), lustre@internals@vdom:element(RGD)}.

-type element_diff(RGE) :: {element_diff,
        gleam@dict:dict(binary(), lustre@internals@vdom:element(RGE)),
        gleam@set:set(binary()),
        gleam@dict:dict(binary(), attribute_diff(RGE)),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, RGE} |
            {error, list(gleam@dynamic:decode_error())}))}.

-type attribute_diff(RGF) :: {attribute_diff,
        gleam@set:set(lustre@internals@vdom:attribute(RGF)),
        gleam@set:set(binary()),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, RGF} |
            {error, list(gleam@dynamic:decode_error())}))}.

-spec do_attribute(
    attribute_diff(RHE),
    binary(),
    {ok, lustre@internals@vdom:attribute(RHE)} | {error, nil},
    {ok, lustre@internals@vdom:attribute(RHE)} | {error, nil}
) -> attribute_diff(RHE).
...
-spec zip(list(RHV), list(RHV)) -> list({gleam@option:option(RHV),
    gleam@option:option(RHV)}).
zip(Xs, Ys) ->
    case {Xs, Ys} of
        {[], []} ->
            [];
...
-spec event_handler(lustre@internals@vdom:attribute(RIH)) -> {ok,
        {binary(),
            fun((gleam@dynamic:dynamic_()) -> {ok, RIH} |
                {error, list(gleam@dynamic:decode_error())})}} |
    {error, nil}.
...
-spec attribute_dict(list(lustre@internals@vdom:attribute(RIB))) -> gleam@dict:dict(binary(), lustre@internals@vdom:attribute(RIB)).
...
-spec do_element_list(
    element_diff(RGR),
    list(lustre@internals@vdom:element(RGR)),
    list(lustre@internals@vdom:element(RGR)),
    binary()
) -> element_diff(RGR).
...
-spec do_elements(
    element_diff(RGK),
    gleam@option:option(lustre@internals@vdom:element(RGK)),
    gleam@option:option(lustre@internals@vdom:element(RGK)),
    binary()
) -> element_diff(RGK).
...
-spec fold_element_list_event_handlers(
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, RIU} |
        {error, list(gleam@dynamic:decode_error())})),
...
-spec fold_event_handlers(
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, RIM} |
        {error, list(gleam@dynamic:decode_error())})),
