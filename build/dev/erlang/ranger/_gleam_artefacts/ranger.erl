...
-spec create(
    ...
    fun((FTN) -> FTN),
    fun((FTM, FTN) -> FTM),
    fun((FTM, FTM) -> gleam@order:order())
) -> fun((FTM, FTM, FTN) -> {ok, gleam@iterator:iterator(FTM)} | {error, nil}).
...
-spec create_infinite(
    fun((FTR) -> boolean()),
    fun((FTR, FTS) -> FTR),
    fun((FTR, FTR) -> gleam@order:order())
) -> fun((FTR, FTS) -> {ok, gleam@iterator:iterator(FTR)} | {error, nil}).
