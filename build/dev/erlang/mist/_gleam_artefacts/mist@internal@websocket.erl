...
-type valid_message(XET) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, XET}.

-type websocket_message(XEU) :: {valid, valid_message(XEU)} | invalid.
...
-type handler_message(XEV) :: {internal, gramps@websocket:frame()} | {user, XEV}.

-type websocket_state(XEW) :: {websocket_state,
        bitstring(),
        XEW,
...
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(XGE))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(XGE))).
...
-spec apply_frames(
    ...
    fun((XFU, websocket_connection(), handler_message(XFV)) -> gleam@otp@actor:next(XFV, XFU)),
    websocket_connection(),
    gleam@otp@actor:next(websocket_message(XFV), XFU),
    fun((XFU) -> nil)
) -> gleam@otp@actor:next(websocket_message(XFV), XFU).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {XFF,
        gleam@option:option(gleam@erlang@process:selector(XFG))}),
    fun((XFF) -> nil),
    fun((XFF, websocket_connection(), handler_message(XFG)) -> gleam@otp@actor:next(XFG, XFF)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok, gleam@erlang@process:subject(websocket_message(XFG))} | {error, nil}.
