...
type Model {
  Model(active_cell: String, grid: Dict(String, parser.FormExpr), error: Option(interpreter.InterpretError))
}
...
fn update(model: Model, msg: Msg) -> #(Model, effect.Effect(Msg)) {
  case msg {
    UserSetCellValue(key, val) -> {
      let res = {
        use tokens <- result.try(scanner.scan(val) |> result.map_error(interpreter.ScanError))
        use expr <- result.try(parser.parse(tokens) |> result.map_error(interpreter.ParseError))
        Ok(expr)
      }
      case res {
        Ok(val) -> #(
        ...
        Error(e) -> #(
          Model(..model, error: Some(e)), effect.none()
        )
      }
...
fn view(model: Model) -> element.Element(Msg) {
  let cells =
    ...
    |> list.map(fn(c) {
      html.input([event.on_input(UserSetCellValue(key: c, val: _)), event.on_click(UserClickedCell(c))])
    })
