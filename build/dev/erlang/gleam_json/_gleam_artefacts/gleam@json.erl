...
-spec decode_bits(
    bitstring(),
    fun((gleam@dynamic:dynamic_()) -> {ok, IBD} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, IBD} | {error, decode_error()}.
...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, IAZ} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, IAZ} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, IAV} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, IAV} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
...
-spec nullable(gleam@option:option(IBJ), fun((IBJ) -> json())) -> json().
...
-spec array(list(IBN), fun((IBN) -> json())) -> json().
