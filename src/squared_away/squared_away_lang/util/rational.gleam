...
pub fn from_string(input: String) -> Result(#(Rat, String), Nil) {
    ...
    case rest {
        "." <> rest -> {
            ...
            use multiplier <- result.try(bigi.power(bigi.from_int(10), bigi.from_int(string.length(decimal))))
            use decimal <- result.try(bigi.from_string(decimal))
            Ok(#(simplify(Rat(bigi.add(bigi.multiply(whole, multiplier), decimal), multiplier)), rest))
        }
...
pub fn add(lhs: Rat, rhs: Rat) -> Rat {
    let Rat(n1, d1) = lhs 
    let Rat(n2, d2) = rhs 
    simplify(Rat(bigi.add(bigi.multiply(n1, d2), bigi.multiply(n2, d1)), bigi.multiply(d1, d2)))
}

pub fn subtract(lhs: Rat, rhs: Rat) -> Rat {
    let Rat(n1, d1) = lhs 
    let Rat(n2, d2) = rhs 
    simplify(Rat(bigi.subtract(bigi.multiply(n1, d2), bigi.multiply(n2, d1)), bigi.multiply(d1, d2)))
}
...
fn do_to_string(precision: Int, remainder: bigi.BigInt, d: bigi.BigInt, acc: String) -> String {
    case precision {
        0 -> acc 
        _ -> {
