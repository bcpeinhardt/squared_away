...
-opaque cli(ADTG) :: {cli,
        fun((env()) -> {env(),
            {ok, ADTG} | {error, lustre_dev_tools@error:error()}})}.
...
-spec run(cli(ADTH), glint:flags()) -> {ok, ADTH} |
...
-spec from_result({ok, ADTP} | {error, lustre_dev_tools@error:error()}) -> cli(ADTP).
from_result(Result) ->
    {cli, fun(Env) -> {Env, Result} end}.

-spec do(cli(ADTT), fun((ADTT) -> cli(ADTV))) -> cli(ADTV).
do(Step, Next) ->
    {cli,
        fun(Env) ->
            {Env@1, Result} = (erlang:element(2, Step))(Env),
...
-spec in(fun(() -> ADTY)) -> cli(ADTY).
in(Value) ->
    {cli, fun(Env) -> {Env, {ok, Value()}} end}.

-spec map(cli(ADUA), fun((ADUA) -> ADUC)) -> cli(ADUC).
map(Step, Next) ->
    {cli,
        fun(Env) ->
            {Env@1, Result} = (erlang:element(2, Step))(Env),
...
-spec 'try'(
    {ok, ADUE} | {error, lustre_dev_tools@error:error()},
    fun((ADUE) -> cli(ADUH))
) -> cli(ADUH).
'try'(Result, Next) ->
...
-spec log(binary(), fun(() -> cli(ADUK))) -> cli(ADUK).
log(Message, Next) ->
    {cli,
        fun(Env) ->
...
-spec success(binary(), fun(() -> cli(ADUN))) -> cli(ADUN).
...
-spec notify(binary(), fun(() -> cli(ADUQ))) -> cli(ADUQ).
notify(Message, Next) ->
    {cli,
        fun(Env) ->
            Env@1 = erlang:setelement(3, Env, case erlang:element(3, Env) of
...
-spec template(binary(), fun((binary()) -> cli(ADUV))) -> cli(ADUV).
template(Name, Next) ->
    {cli,
        fun(Env) ->
...
-spec get_config_value(
    ...
    ADVB,
    list(binary()),
    fun((gleam@dict:dict(binary(), tom:toml()), list(binary())) -> {ok, ADVB} |
        {error, any()}),
    fun((glint:flags()) -> {ok, ADVB} | {error, any()})
) -> cli(ADVB).
