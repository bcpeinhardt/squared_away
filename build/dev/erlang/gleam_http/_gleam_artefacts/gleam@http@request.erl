...
-spec set_header(request(JWG), binary(), binary()) -> request(JWG).
...
-spec set_query(request(JXC), list({binary(), binary()})) -> request(JXC).
set_query(Req, Query) ->
    Pair = fun(T) ->
...
-spec set_method(request(JXG), gleam@http:method()) -> request(JXG).
...
-spec set_scheme(request(JXN), gleam@http:scheme()) -> request(JXN).
set_scheme(Req, Scheme) ->
    erlang:setelement(5, Req, Scheme).

-spec set_host(request(JXQ), binary()) -> request(JXQ).
set_host(Req, Host) ->
    erlang:setelement(6, Req, Host).

-spec set_port(request(JXT), integer()) -> request(JXT).
set_port(Req, Port) ->
    erlang:setelement(7, Req, {some, Port}).

-spec set_path(request(JXW), binary()) -> request(JXW).
set_path(Req, Path) ->
    erlang:setelement(8, Req, Path).

-spec set_cookie(request(JXZ), binary(), binary()) -> request(JXZ).
