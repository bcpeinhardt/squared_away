...
-opaque glint(MRG) :: {glint, config(), command_node(MRG)}.

-type args_count() :: {eq_args, integer()} | {min_args, integer()}.

-opaque command(MRH) :: {command,
        fun((named_args(), list(binary()), flags()) -> MRH),
        flags(),
        binary(),
        gleam@option:option(args_count()),
        list(binary())}.

-type internal_command(MRI) :: {internal_command,
        fun((named_args(), list(binary()), flags()) -> MRI),
        flags(),
        gleam@option:option(args_count()),
        list(binary())}.
...
-type command_node(MRJ) :: {command_node,
        gleam@option:option(internal_command(MRJ)),
        gleam@dict:dict(binary(), command_node(MRJ)),
        flags(),
        binary()}.

-type out(MRK) :: {out, MRK} | {help, binary()}.
...
-opaque flag(MRL) :: {flag,
        binary(),
        binary(),
        fun((binary()) -> {ok, MRL} | {error, snag:snag()}),
        fun((flag_internals(MRL)) -> value()),
        fun((flags(), binary()) -> {ok, MRL} | {error, snag:snag()}),
        gleam@option:option(MRL)}.

-type flag_internals(MRM) :: {flag_internals,
        gleam@option:option(MRM),
        fun((binary()) -> {ok, MRM} | {error, snag:snag()})}.

-type flag_entry() :: {flag_entry, value(), binary()}.

-opaque flags() :: {flags, gleam@dict:dict(binary(), flag_entry())}.

-spec config(glint(MRT), config()) -> glint(MRT).
config(Glint, Config) ->
    erlang:setelement(2, Glint, Config).

-spec pretty_help(glint(MRW), pretty_help()) -> glint(MRW).
...
-spec as_module(glint(MSF)) -> glint(MSF).
as_module(Glint) ->
    config(Glint, erlang:setelement(4, erlang:element(2, Glint), true)).

-spec with_indent_width(glint(MSI), integer()) -> glint(MSI).
...
-spec with_max_output_width(glint(MSL), integer()) -> glint(MSL).
...
-spec with_min_first_column_width(glint(MSO), integer()) -> glint(MSO).
...
-spec with_column_gap(glint(MSR), integer()) -> glint(MSR).
...
-spec global_help(glint(MTA), binary()) -> glint(MTA).
...
-spec command_help(binary(), fun(() -> command(MTP))) -> command(MTP).
command_help(Desc, F) ->
    erlang:setelement(4, F(), Desc).

-spec unnamed_args(args_count(), fun(() -> command(MTS))) -> command(MTS).
unnamed_args(Count, F) ->
    erlang:setelement(5, F(), {some, Count}).

-spec named_arg(
    binary(),
    fun((fun((named_args()) -> binary())) -> command(MTV))
) -> command(MTV).
named_arg(Name, F) ->
    Cmd = (F(
        fun(Named_args) ->
...
-spec new_builder(
    binary(),
    fun((flag_internals(MWK)) -> value()),
    fun((flags(), binary()) -> {ok, MWK} | {error, snag:snag()}),
    fun((binary()) -> {ok, MWK} | {error, snag:snag()})
) -> flag(MWK).
...
-spec attempt(
    {ok, MXC} | {error, MXD},
    fun((MXC) -> {ok, any()} | {error, MXD})
) -> {ok, MXC} | {error, MXD}.
attempt(Val, F) ->
    gleam@result:'try'(Val, fun(A) -> gleam@result:replace(F(A), A) end).

-spec wrap_with_constraint(
    fun((binary()) -> {ok, MWW} | {error, snag:snag()}),
    fun((MWW) -> {ok, MWW} | {error, snag:snag()})
) -> fun((binary()) -> {ok, MWW} | {error, snag:snag()}).
wrap_with_constraint(P, Constraint) ->
    fun(Input) -> attempt(P(Input), Constraint) end.

-spec flag_constraint(flag(MWS), fun((MWS) -> {ok, MWS} | {error, snag:snag()})) -> flag(MWS).
...
-spec flag_help(flag(MXL), binary()) -> flag(MXL).
flag_help(Flag, Description) ->
    erlang:setelement(3, Flag, Description).

-spec flag_default(flag(MXO), MXO) -> flag(MXO).
...
-spec flag(
    flag(MTY),
    fun((fun((flags()) -> {ok, MTY} | {error, snag:snag()})) -> command(MUB))
) -> command(MUB).
flag(Flag, F) ->
    Cmd = F(
        fun(_capture) ->
...
-spec fold(flags(), MXR, fun((MXR, binary(), flag_entry()) -> MXR)) -> MXR.
fold(Flags, Acc, F) ->
...
-spec command(fun((named_args(), list(binary()), flags()) -> MTM)) -> command(MTM).
command(Runner) ->
...
-spec get_value(
    flags(),
    binary(),
    fun((flag_entry()) -> {ok, MYE} | {error, snag:snag()})
) -> {ok, MYE} | {error, snag:snag()}.
...
-spec get_flag(flags(), flag(MYH)) -> {ok, MYH} | {error, snag:snag()}.
get_flag(Flags, Flag) ->
...
-spec do_update_at(
    command_node(MZA),
    list(binary()),
    fun((command_node(MZA)) -> command_node(MZA))
) -> command_node(MZA).
...
-spec update_at(
    glint(MYU),
    list(binary()),
    fun((command_node(MYU)) -> command_node(MYU))
) -> glint(MYU).
...
-spec path_help(glint(MSW), list(binary()), binary()) -> glint(MSW).
...
-spec add(glint(MTD), list(binary()), command(MTD)) -> glint(MTD).
...
-spec group_flag(glint(MUE), list(binary()), flag(any())) -> glint(MUE).
...
-spec execute(glint(MUK), list(binary())) -> {ok, out(MUK)} | {error, binary()}.
execute(Glint, Args) ->
    Help_flag = <<"--"/utf8,
        (erlang:element(
            2,
...
-spec run_and_handle(glint(MVK), list(binary()), fun((MVK) -> any())) -> nil.
