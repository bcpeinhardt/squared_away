...
-spec equal(MGS, MGS) -> nil.
equal(A, B) ->
    gleeunit_ffi:should_equal(A, B).

-spec not_equal(MGT, MGT) -> nil.
not_equal(A, B) ->
    gleeunit_ffi:should_not_equal(A, B).

-spec be_ok({ok, MGU} | {error, any()}) -> MGU.
be_ok(A) ->
    gleeunit_ffi:should_be_ok(A).

-spec be_error({ok, any()} | {error, MGZ}) -> MGZ.
be_error(A) ->
    gleeunit_ffi:should_be_error(A).

-spec be_some(gleam@option:option(MHC)) -> MHC.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
