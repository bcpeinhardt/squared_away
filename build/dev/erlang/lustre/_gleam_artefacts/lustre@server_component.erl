...
-spec component(list(lustre@internals@vdom:attribute(UYT))) -> lustre@internals@vdom:element(UYT).
component(Attrs) ->
...
-spec subscribe(binary(), fun((lustre@internals@patch:patch(UZG)) -> nil)) -> lustre@internals@runtime:action(UZG, lustre:server_component()).
...
-spec do_set_selector(
    gleam@erlang@process:selector(lustre@internals@runtime:action(any(), UZW))
) -> lustre@effect:effect(UZW).
do_set_selector(Sel) ->
    lustre@effect:from(
        fun(_) ->
...
-spec set_selector(
    gleam@erlang@process:selector(lustre@internals@runtime:action(any(), UZQ))
) -> lustre@effect:effect(UZQ).
set_selector(Sel) ->
    do_set_selector(Sel).
