...
-type spec(LBZ, LCA) :: {spec,
        LBZ,
        integer(),
        integer(),
        fun((children(LBZ)) -> children(LCA))}.

-opaque children(LCB) :: {ready, starter(LCB)} | {failed, child_start_error()}.

-opaque child_spec(LCC, LCD, LCE) :: {child_spec,
        fun((LCD) -> {ok, gleam@erlang@process:subject(LCC)} |
            {error, gleam@otp@actor:start_error()}),
        fun((LCD, gleam@erlang@process:subject(LCC)) -> LCE)}.
...
-spec start_child(child_spec(any(), LCL, LCM), LCL) -> {ok, child(LCM)} |
    {error, child_start_error()}.
...
-spec perform_instruction_for_child(
    LCZ,
    instruction(),
    child_spec(any(), LCZ, LDB),
    child(LDB)
) -> {ok, {child(LDB), instruction()}} | {error, child_start_error()}.
...
-spec start_and_add_child(starter(LDS), child_spec(any(), LDS, LDV)) -> children(LDV).
...
-spec add(children(LEA), child_spec(any(), LEA, LED)) -> children(LED).
...
-spec supervisor(
    fun((LEI) -> {ok, gleam@erlang@process:subject(LEJ)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(LEJ, LEI, LEI).
supervisor(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-spec worker(
    fun((LEQ) -> {ok, gleam@erlang@process:subject(LER)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(LER, LEQ, LEQ).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-spec returning(
    child_spec(LEY, LEZ, any()),
    fun((LEZ, gleam@erlang@process:subject(LEY)) -> LFF)
) -> child_spec(LEY, LEZ, LFF).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-spec init(spec(any(), LFK)) -> gleam@otp@actor:init_result(state(LFK), message()).
init(Spec) ->
...
-spec handle_exit(gleam@erlang@process:pid_(), state(LFQ)) -> gleam@otp@actor:next(message(), state(LFQ)).
...
-spec loop(message(), state(LFV)) -> gleam@otp@actor:next(message(), state(LFV)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
