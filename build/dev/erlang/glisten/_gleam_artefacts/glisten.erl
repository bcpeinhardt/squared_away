...
-type message(PCY) :: {packet, bitstring()} | {user, PCY}.
...
-type connection(PCZ) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(PCZ))}.

-opaque handler(PDA, PDB) :: {handler,
        fun((connection(PDA)) -> {PDB,
            gleam@option:option(gleam@erlang@process:selector(PDA))}),
        fun((message(PDA), PDB, connection(PDA)) -> gleam@otp@actor:next(message(PDA), PDB)),
        gleam@option:option(fun((PDB) -> nil)),
        integer(),
        boolean()}.
...
-spec convert_on_init(
    fun((connection(PEG)) -> {PEI,
        gleam@option:option(gleam@erlang@process:selector(PEG))})
) -> fun((glisten@internal@handler:connection(PEG)) -> {PEI,
    gleam@option:option(gleam@erlang@process:selector(PEG))}).
...
-spec handler(
    fun((connection(PEO)) -> {PEQ,
        gleam@option:option(gleam@erlang@process:selector(PEO))}),
    fun((message(PEO), PEQ, connection(PEO)) -> gleam@otp@actor:next(message(PEO), PEQ))
) -> handler(PEO, PEQ).
handler(On_init, Loop) ->
    {handler, On_init, Loop, none, 10, false}.

-spec map_user_selector(gleam@erlang@process:selector(message(PDV))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(PDV)).
...
-spec convert_loop(
    fun((message(PEA), PEB, connection(PEA)) -> gleam@otp@actor:next(message(PEA), PEB))
) -> fun((glisten@internal@handler:loop_message(PEA), PEB, glisten@internal@handler:connection(PEA)) -> gleam@otp@actor:next(glisten@internal@handler:loop_message(PEA), PEB)).
convert_loop(Loop) ->
    fun(Msg, Data, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec with_close(handler(PEX, PEY), fun((PEY) -> nil)) -> handler(PEX, PEY).
with_close(Handler, On_close) ->
    erlang:setelement(4, Handler, {some, On_close}).

-spec with_pool_size(handler(PFD, PFE), integer()) -> handler(PFD, PFE).
with_pool_size(Handler, Size) ->
    erlang:setelement(5, Handler, Size).

-spec with_http2(handler(PFJ, PFK)) -> handler(PFJ, PFK).
with_http2(Handler) ->
