...
-type message(KNJ) :: {message, KNJ} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(KNK, KNL) :: {continue,
        KNL,
        gleam@option:option(gleam@erlang@process:selector(KNK))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(KNM, KNN) :: {ready, KNM, gleam@erlang@process:selector(KNN)} |
    {failed, binary()}.

-type self(KNO, KNP) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        KNO,
        gleam@erlang@process:subject(KNP),
        gleam@erlang@process:selector(message(KNP)),
        gleam@otp@system:debug_state(),
        fun((KNP, KNO) -> next(KNP, KNO))}.

-type spec(KNQ, KNR) :: {spec,
        fun(() -> init_result(KNQ, KNR)),
        integer(),
        fun((KNR, KNQ) -> next(KNR, KNQ))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(KNS) :: {ack,
        {ok, gleam@erlang@process:subject(KNS)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-spec continue(KNZ) -> next(any(), KNZ).
continue(State) ->
    {continue, State, none}.

-spec with_selector(next(KOD, KOE), gleam@erlang@process:selector(KOD)) -> next(KOD, KOE).
...
-spec selecting_system_messages(gleam@erlang@process:selector(message(KOP))) -> gleam@erlang@process:selector(message(KOP)).
...
-spec receive_message(self(any(), KOL)) -> message(KOL).
...
-spec init_selector(
    gleam@erlang@process:subject(KTD),
    gleam@erlang@process:selector(KTD)
) -> gleam@erlang@process:selector(message(KTD)).
...
-spec initialise_actor(
    spec(any(), KPG),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(KPG)} |
        {error, gleam@erlang@process:exit_reason()})
...
-spec start_spec(spec(any(), KPT)) -> {ok, gleam@erlang@process:subject(KPT)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-spec start(KPZ, fun((KQA, KPZ) -> next(KQA, KPZ))) -> {ok,
        gleam@erlang@process:subject(KQA)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-spec send(gleam@erlang@process:subject(KQG), KQG) -> nil.
send(Subject, Msg) ->
    gleam@erlang@process:send(Subject, Msg).

-spec call(
    gleam@erlang@process:subject(KQI),
    fun((gleam@erlang@process:subject(KQK)) -> KQI),
    integer()
) -> KQK.
