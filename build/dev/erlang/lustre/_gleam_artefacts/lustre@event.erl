...
-spec on(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, UTR} |
        {error, list(gleam@dynamic:decode_error())})
) -> lustre@internals@vdom:attribute(UTR).
on(Name, Handler) ->
    lustre@attribute:on(Name, Handler).

-spec on_click(UTU) -> lustre@internals@vdom:attribute(UTU).
on_click(Msg) ->
    on(<<"click"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_down(UTW) -> lustre@internals@vdom:attribute(UTW).
on_mouse_down(Msg) ->
    on(<<"mousedown"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_up(UTY) -> lustre@internals@vdom:attribute(UTY).
on_mouse_up(Msg) ->
    on(<<"mouseup"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_enter(UUA) -> lustre@internals@vdom:attribute(UUA).
on_mouse_enter(Msg) ->
    on(<<"mouseenter"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_leave(UUC) -> lustre@internals@vdom:attribute(UUC).
on_mouse_leave(Msg) ->
    on(<<"mouseleave"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_over(UUE) -> lustre@internals@vdom:attribute(UUE).
on_mouse_over(Msg) ->
    on(<<"mouseover"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_out(UUG) -> lustre@internals@vdom:attribute(UUG).
on_mouse_out(Msg) ->
    on(<<"mouseout"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_keypress(fun((binary()) -> UUI)) -> lustre@internals@vdom:attribute(UUI).
on_keypress(Msg) ->
...
-spec on_keydown(fun((binary()) -> UUK)) -> lustre@internals@vdom:attribute(UUK).
on_keydown(Msg) ->
...
-spec on_keyup(fun((binary()) -> UUM)) -> lustre@internals@vdom:attribute(UUM).
on_keyup(Msg) ->
...
-spec on_focus(UUU) -> lustre@internals@vdom:attribute(UUU).
on_focus(Msg) ->
    on(<<"focus"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_blur(UUW) -> lustre@internals@vdom:attribute(UUW).
on_blur(Msg) ->
...
-spec on_input(fun((binary()) -> UUO)) -> lustre@internals@vdom:attribute(UUO).
on_input(Msg) ->
...
-spec on_check(fun((boolean()) -> UUQ)) -> lustre@internals@vdom:attribute(UUQ).
on_check(Msg) ->
...
-spec on_submit(UUS) -> lustre@internals@vdom:attribute(UUS).
on_submit(Msg) ->
    on(
        <<"submit"/utf8>>,
        fun(Event) ->
