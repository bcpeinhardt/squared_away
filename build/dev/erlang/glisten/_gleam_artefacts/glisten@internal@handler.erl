...
-type message(OKZ) :: {internal, internal_message()} | {user, OKZ}.

-type loop_message(OLA) :: {packet, bitstring()} | {custom, OLA}.

-type loop_state(OLB, OLC) :: {loop_state,
        {ok, {glisten@transport:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(OLB)),
        glisten@transport:transport(),
        OLC}.

-type connection(OLD) :: {connection,
        {ok, {glisten@transport:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(OLD))}.

-type handler(OLE, OLF) :: {handler,
        glisten@socket:socket(),
        fun((loop_message(OLE), OLF, connection(OLE)) -> gleam@otp@actor:next(loop_message(OLE), OLF)),
        fun((connection(OLE)) -> {OLF,
            gleam@option:option(gleam@erlang@process:selector(OLE))}),
        gleam@option:option(fun((OLF) -> nil)),
        glisten@transport:transport()}.

-spec start(handler(OLP, any())) -> {ok,
        gleam@erlang@process:subject(message(OLP))} |
                ...
                Selector = begin
                    ...
                    _pipe@5 = gleam@erlang@process:selecting_record3(
                        ...
                        fun(_, Data) -> _pipe@2 = Data,
                            _pipe@3 = gleam@dynamic:bit_array(_pipe@2),
                    ...
                    _pipe@9 = gleam@erlang@process:selecting_record3(
                        ...
                        fun(_, Data@1) -> _pipe@6 = Data@1,
                            _pipe@7 = gleam@dynamic:bit_array(_pipe@6),
