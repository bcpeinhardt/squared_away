...
-opaque task(LOV) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:process_monitor(),
        gleam@erlang@process:selector(message(LOV))}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic_()}.

-type message(LOW) :: {from_monitor, gleam@erlang@process:process_down()} |
    {from_subject, LOW}.

-spec async(fun(() -> LOX)) -> task(LOX).
async(Work) ->
    Owner = erlang:self(),
...
-spec try_await(task(LPB), integer()) -> {ok, LPB} | {error, await_error()}.
...
-spec await(task(LPF), integer()) -> LPF.
await(Task, Timeout) ->
...
-spec try_await_forever(task(LPH)) -> {ok, LPH} | {error, await_error()}.
...
-spec await_forever(task(LPL)) -> LPL.
await_forever(Task) ->
