...
-type state(SAD, SAE, SAF) :: {state,
        gleam@erlang@process:subject(action(SAE, SAF)),
        SAD,
        fun((SAD, SAE) -> {SAD, lustre@effect:effect(SAE)}),
        fun((SAD) -> lustre@internals@vdom:element(SAE)),
        lustre@internals@vdom:element(SAE),
        gleam@dict:dict(binary(), fun((lustre@internals@patch:patch(SAE)) -> nil)),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, SAE} |
            {error, list(gleam@dynamic:decode_error())})),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, SAE} |
            {error, list(gleam@dynamic:decode_error())}))}.

-type action(SAG, SAH) :: {attrs, list({binary(), gleam@dynamic:dynamic_()})} |
    {batch, list(SAG), lustre@effect:effect(SAG)} |
    {debug, debug_action()} |
    {dispatch, SAG} |
    {emit, binary(), gleam@json:json()} |
    {event, binary(), gleam@dynamic:dynamic_()} |
    {set_selector, gleam@erlang@process:selector(action(SAG, SAH))} |
    shutdown |
    {subscribe, binary(), fun((lustre@internals@patch:patch(SAG)) -> nil)} |
    {unsubscribe, binary()}.
...
-spec run_renderers(
    gleam@dict:dict(any(), fun((lustre@internals@patch:patch(SBM)) -> nil)),
    lustre@internals@patch:patch(SBM)
) -> nil.
run_renderers(Renderers, Patch) ->
    gleam@dict:fold(Renderers, nil, fun(_, _, Renderer) -> Renderer(Patch) end).

-spec run_effects(
    lustre@effect:effect(SBR),
    gleam@erlang@process:subject(action(SBR, any()))
) -> nil.
...
-spec loop(action(SAW, SAX), state(SBA, SAW, SAX)) -> gleam@otp@actor:next(action(SAW, SAX), state(SBA, SAW, SAX)).
loop(Message, State) ->
    case Message of
        {attrs, Attrs} ->
...
-spec start(
    {SAI, lustre@effect:effect(SAJ)},
    fun((SAI, SAJ) -> {SAI, lustre@effect:effect(SAJ)}),
    fun((SAI) -> lustre@internals@vdom:element(SAJ)),
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, SAJ} |
        {error, list(gleam@dynamic:decode_error())}))
) -> {ok, gleam@erlang@process:subject(action(SAJ, any()))} |
