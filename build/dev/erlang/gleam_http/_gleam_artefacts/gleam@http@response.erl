...
-type response(KDA) :: {response, integer(), list({binary(), binary()}), KDA}.
...
-spec set_header(response(KDP), binary(), binary()) -> response(KDP).
...
-spec prepend_header(response(KDS), binary(), binary()) -> response(KDS).
prepend_header(Response, Key, Value) ->
    Headers = [{gleam@string:lowercase(Key), Value} |
        erlang:element(3, Response)],
    erlang:setelement(3, Response, Headers).

-spec set_body(response(any()), KDX) -> response(KDX).
set_body(Response, Body) ->
    {response, Status, Headers, _} = Response,
    {response, Status, Headers, Body}.

-spec try_map(response(KDB), fun((KDB) -> {ok, KDD} | {error, KDE})) -> {ok,
        response(KDD)} |
    {error, KDE}.
...
-spec map(response(KDZ), fun((KDZ) -> KEB)) -> response(KEB).
...
-spec expire_cookie(response(KEJ), binary(), gleam@http@cookie:attributes()) -> response(KEJ).
